...
def get_project_vm_size(
    ...
    return {"vm_size": config_client.get_project_vm_size(project_slug)}
...
def edit_project_vm_size(
    ...
    a VM size category."""
    config_client.set_project_vm_size(project_slug, vm_size or None)


@router.get(
    "/{project_slug}/image-definition",
    status_code=200,
    dependencies=[Depends(verify_admin_permission)],
)
...
def edit_project_image_definition(
    ...
    config_client.set_project_image_definition(project_slug, image_definition or None)
